...
parse_token() {
  ...
  if [[ "$DATETIME" ]] ; then
    ...
    echo -n "$t2" | base64 --d 2>/dev/null | sed  's/\(".\{10\}T.\{14\}"\)/"YYYY-MM-DDTHH24:MI:SSTZ"/g' | jq -S '.exp = 0'
  fi
}
...
EOF
  ...
  if [[ "$DATETIME" ]] ; then
    ...
    result=$(echo "$resp" | sed  's/\(".\{10\}T.\{14\}"\)/"YYYY-MM-DDTHH24:MI:SSTZ"/g' | jq -S '.'  || echo "ERROR: $resp")
  fi
  if [[ "$new_key" == *=* ]] ; then
